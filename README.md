...
| A fix from `git pull` seems to do nothing | The setup page shows a red bar when the server answering it is older than the page. Restart `splatworld run`; if it persists, `python -m pip install -e ./server`. |

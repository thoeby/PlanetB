...
# three.js (MIT): the frame atom's renderer (client/lib/raster.js). The
# ES-module build and GLTFLoader import each other by bare specifier and a Web
# Worker has no import map, so the specifiers are rewritten to the flat layout
# under client/vendor/three/. Checked in: these files are what runs.
vendor_three () {
    local dest=client/vendor/three tmp
    [ -f "$dest/three.module.js" ] && [ "${FORCE:-}" != "1" ] && {
        echo "vendor: $dest is already here (FORCE=1 to refetch)"; return 0; }
    tmp=$(mktemp -d)
    ( cd "$tmp" && npm pack "three@${THREE_VERSION}" --silent > /dev/null ) || {
        echo "vendor: three unreachable, skipping"; rm -rf "$tmp"; return 0; }
    mkdir -p "$dest" "$tmp/three"
    ...
    sed -i -e "s#from 'three'#from './three.module.js'#" \
           ...
           "$dest"/*.js
    printf 'three %s, MIT\n' "$THREE_VERSION" > "$dest/NOTICE"
    rm -rf "$tmp"
    echo "vendor: three $THREE_VERSION from npm"
}

-- 0097_thecamerasstandontheground.sql — the cameras stand on the ground.
--
-- `frame-v3` (client/atoms/frame.js, client/lib/cameras.js): frame-v2's
-- street cameras stood 1.7 m above the tile's mean height, which on any
-- slope is underground, and its rings at 15 degrees looked through the hill
-- in front of them. Every pose now stands on the height.r16 `assemble`
-- wrote, and the traced frame is denoised along the surfaces
-- (client/lib/denoise.js) before it is stored. Same DAG, same params; the
-- algo_version is what says the bytes differ (Invariant 2).
...
BEGIN
    ...
    IF a_z >= 16 THEN
        i := 0;
        WHILE i < views LOOP
            frames := frames || new_atom(a_job, 'frame', 'frame-v3',

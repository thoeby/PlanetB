-- 0117_theframesarenotstriped.sql — the frames the trainer learns from are
-- drawn without the shadow acne.
--
-- What every training image had on it: regular diagonal bands across the
-- ground, each made of short vertical dashes. Those are the shadow map's
-- texel rows and columns, projected obliquely onto the terrain. frame-v6 had
-- the ground cast both of its faces into the shadow map, so every sunlit point
-- compared its depth against its own and shadowed itself wherever that
-- rounded the wrong way — over a 4096 map stretched across a 2.4 km tile,
-- which is everywhere. A splat trained on that learns the stripes.
--
-- frame-v7 (client/lib/raster.js) lets the map hold the far slopes only,
-- which is three.js's default and the right thing for a heightfield. Same
-- cameras, same light, same everything else; a new algo_version because the
-- bytes differ (Invariant 2), so every tile is framed again.
--
-- db/0116's build_dag, with the frames at v7.

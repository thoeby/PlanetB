-- 0123_theframesarenotstripedthistime.sql — the frames are drawn without
-- the shadow acne, by the thing that actually ends it.
--
-- frame-v7 (db/0117) changed which side of the ground the shadow pass draws.
-- A heightfield has one side: its back face is the same triangles at the same
-- depth, and every sunlit point still compared against itself. The stripes
-- stayed. What ends acne on a variance shadow map is a normal bias: the
-- lookup moved off the surface along its normal by a texel and a half, so a
-- surface never finds itself, and no depth bias pushing the other way.
-- frame-v8 (client/lib/raster.js). New bytes, new algo_version (Invariant 2).
--
-- db/0122's build_dag, with the frames at v8.
...
BEGIN
    IF a_z >= 14 THEN
        ...
        i := 0;
        WHILE i < views LOOP
            frames := frames || new_atom(a_job, 'frame', 'frame-v8',

-- 0101_moreiterationsforthecard.sql — a trained tile gets the iterations it
-- needs.
--
-- train-v3 seeds half the budget and lets brush densify to the rest
-- (client/lib/brush.js): splitting where the picture is wrong is what puts
-- small splats on edges. That needs iterations — 1 500 was a quarter of what
-- a gaussian-splat run is normally given — and a card that does a step in a
-- fraction of a second has them: 4 000 at z16, 5 000 at z18.
    ...
    IF a_z >= 16 THEN
        ...
        trn := new_atom(a_job, 'train', 'train-v3',
            ...
            jsonb_build_object('budget', budget,
                'iters', CASE WHEN a_z = 18 THEN 5000 ELSE 4000 END,
                'size', px,
                'camera_set', cams,
                'needs_webgpu', true,

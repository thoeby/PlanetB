...
-- names (assemble-v7, frame-v10), the iteration count (db/0114), and the
...
SELECT is((SELECT algo_version FROM atom
           WHERE job_id = (SELECT j18 FROM jobs) AND op = 'assemble'), 'assemble-v7',
...
SELECT is((SELECT algo_version FROM atom
           WHERE job_id = (SELECT j14 FROM jobs) AND op = 'assemble'), 'assemble-v7',

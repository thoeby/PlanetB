...
SELECT is((SELECT algo_version FROM atom
           WHERE job_id = (SELECT j18 FROM jobs) AND op = 'train'), 'train-v6',
    'a z18 job trains with train-v6');
...
SELECT is((SELECT algo_version FROM atom
           WHERE job_id = (SELECT j16 FROM jobs) AND op = 'train'), 'train-v6',
    'a z16 job trains with train-v6');
SELECT is((SELECT (params ->> 'iters')::int FROM atom

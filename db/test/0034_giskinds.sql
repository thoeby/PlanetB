...
SELECT has_view('gis', 'f_highway', 'a road layer exists');
SELECT has_view('gis', 'f_natural_point',
                'so does one for a kind db/0034 never had');
SELECT is(
    (SELECT array_agg(column_name::text ORDER BY ordinal_position)
     FROM information_schema.columns
     WHERE table_schema = 'gis' AND table_name = 'f_landuse'),
    (SELECT ARRAY['id', 'area_id', 'rev']
        || array_agg(p.name::text ORDER BY p.ordering, p.name) || ARRAY['geom']
     FROM property p WHERE p.kind = 'landuse'),
...
SELECT is((SELECT (kind, st_ndims(geom)::int) FROM feature), ('highway'::text, 3),
          'the layer sets the kind, the table adds the Z');

UPDATE gis.f_highway SET geom = st_geomfromtext('LINESTRING(7.01 46.01, 7.03 46.03)', 4326);

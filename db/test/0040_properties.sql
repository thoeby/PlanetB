...
SELECT is((SELECT geometry FROM kind WHERE name = 'highway'), 'line',
    'a road is drawn as a line');
SELECT ok((SELECT count(*) FROM property WHERE kind = 'building') >= 4,
...
SELECT throws_ok($$SELECT put_property('landuse', 'invented', 'text')$$,
...
SELECT lives_ok(
    $$SELECT put_property('landuse', 'leaf_type', 'choice',
...
SELECT is((SELECT array_to_string(choices, ',') FROM property
           WHERE kind = 'landuse' AND name = 'leaf_type'),
...
SELECT lives_ok($$INSERT INTO feature (area_id, kind, geom, props) VALUES (
        '00000000-0000-0000-0000-0000000b0003', 'landuse',
...
SELECT throws_ok($$INSERT INTO feature (area_id, kind, geom, props) VALUES (
        '00000000-0000-0000-0000-0000000b0003', 'building',
...
SELECT is(
    (SELECT jsonb_array_length(k -> 'properties')
     ...
     WHERE k ->> 'name' = 'landuse'),
    (SELECT count(*)::int FROM property WHERE kind = 'landuse'),

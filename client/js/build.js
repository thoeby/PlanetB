...
export class Edits {
    ...
    async place(areaId, san, at, pose = {}) {
        const [row] = await this.api.insert('instance', [{
            area_id: areaId, san, lon: at.lon, lat: at.lat, h: at.h ?? 0,
            yaw: pose.yaw ?? 0, pitch: pose.pitch ?? 0, roll: pose.roll ?? 0,
            scale: pose.scale ?? 1,
        }], { select: FIELDS });
        this.stack.push({ undo: 'delete', row });
        this.onChange(row);
        return row;
    }

    // A pose change is one PATCH, and its inverse is the row as it was.

...
export class WorkLoop {
    constructor({ api, filesUrl, caps = {}, spawn = spawnAtomWorker, cache,
    ...
    async compute(atom) {
        ...
        try {
            const out = await worker.run({ atom, inputs },
    ...
    async deliver(atom, out, seconds) {
        ...
        for (const file of out.files) {
            ...
            first ??= sha;
            if (file.ext === out.output) chosen = sha;
            await this.upload(atom, file, sha);
        }
        const output = chosen ?? first;
        const result = { gpu_seconds: seconds, ...out.result };
        return this.api.rpc('submit_atom',
            { atom_id: atom.id, output_sha256: output, result });
    }
    ...
    async upload(atom, file, sha) {
        ...
        if (known) return;
        const dir = file.dir ?? `/jobs/${atom.id}`;
        const res = await this.fetchFn(`${this.filesUrl}${dir}/${sha}.${file.ext}`, {
            method: 'PUT',
            headers: {
                'X-Sha256': sha,
        ...
        if (res.status !== 201 && res.status !== 204) {
            throw new Error(`PUT ${dir}/${sha}.${file.ext} -> ${res.status}`);
        }

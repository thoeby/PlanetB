...
class WfsUrlTest(unittest.TestCase):
    ...
    def test_every_shape_of_address_reaches_wfs(self):
        ...
        for typed in ("localhost:8083/geoserver",
                      "http://localhost:8083/geoserver/",
                      "http://localhost:8083/geoserver/wfs",
                      "http://localhost:8083/geoserver/wfs?service=WFS&request=GetCapabilities"):
            got = wfs_url(typed, "splatworld:feature_footprint", None)
            self.assertTrue(got.startswith("http://localhost:8083/geoserver/wfs?"), got)

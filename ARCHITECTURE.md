...
| `claim_atom(caps) → atom` | `FOR UPDATE SKIP LOCKED` over `state='ready'` and caps satisfied; sets `claimed`, records worker; also reserves the upload path `/jobs/{atom_id}/` |
...
| `submit_verification(atom_id, passed, metrics)` | perceptual; 3 distinct workers all pass → `verified`; any fail → `failed`, worker stats updated |
...
- perceptual: `train` — 3 independent workers render 2 poses and report PSNR ≥ 22. This is probabilistic QA, not proof. Owner's own tab re-verifies its tiles on next visit (free spot check).
- trust: scalar per worker in v1, stats per op recorded for later.
